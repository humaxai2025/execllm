...
Key Metrics:
- Total Models Evaluated: ${execSummary.keyMetrics.totalModels}
- Vendor Diversity: ${execSummary.keyMetrics.vendors} vendor${execSummary.keyMetrics.vendors !== 1 ? 's' : ''}
- Risk Profile: ${execSummary.keyMetrics.avgRisk}
- Enterprise-Ready Models: ${execSummary.keyMetrics.enterpriseCompliance}/${execSummary.keyMetrics.totalModels}
- Primary Industry Focus: ${execSummary.keyMetrics.topIndustry}

Strategic Insights:
${execSummary.insights.map((insight, i) => `${i + 1}. ${insight}`).join('\n')}

Executive Recommendations:
${execSummary.recommendations.map((rec, i) => `${i + 1}. ${rec}`).join('\n')}

Next Steps:
1. Review detailed model analysis below
2. Conduct proof-of-concept with recommended models
3. Establish vendor relationships and pricing discussions
4. Develop implementation timeline and resource allocation
5. Create success metrics and evaluation framework
...
Support our mission: https://buymeacoffee.com/humanxai

Methodology: This comparison uses ExecLLM's executive decision framework, 
evaluating models across business-critical dimensions including strategic fit, 
risk assessment, implementation complexity, and vendor ecosystem.

Disclaimer: Model capabilities and vendor offerings change rapidly. Verify 
current specifications and conduct proof-of-concept testing before making 
...
`;
    ...
    a.href = url;
    a.download = `ExecLLM-Executive-Report-${models.map(m => m.name.replace(/\s+/g, '-')).join('-vs-')}-${new Date().toISOString().split('T')[0]}.txt`;
        ...
        >
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            ...
                            <div className="text-left">
                              <div className="font-medium">Executive Report (.txt)</div>
                              <div className="text-xs text-slate-400">With strategic insights</div>
                            </div>
                          </motion.button>
                          ...
                          >
                            ...
                            <div className="text-left">
                              <div className="font-medium">Executive Report (.html)</div>
                              <div className="text-xs text-slate-400">Full formatted report</div>
                            </div>
                          </motion.button>
                        </div>
                      </motion.div>
          ...
          <div className="p-4 border-t border-slate-700/50 bg-slate-800/30">
            <div className="flex items-center justify-between text-sm text-slate-400">
              <div>
                Export includes comprehensive executive summary with strategic insights
              </div>
              <div>
